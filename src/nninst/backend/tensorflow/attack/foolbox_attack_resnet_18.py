...
from nninst.backend.tensorflow.attack.common import (
    resnet_18_cifar100_real_metrics_per_layer,
)
...
from nninst.backend.tensorflow.model.config import RESNET_18_CIFAR100
from nninst.backend.tensorflow.model.resnet_18_cifar100 import ResNet18Cifar100
from nninst.backend.tensorflow.trace.resnet_18_cifar100_class_trace import (
...
if __name__ == "__main__":
    absolute_threshold, cumulative_threshold, type_ = parse_path_generation_args(RESNET_18_CIFAR100)
    # mode.debug()
    # mode.distributed()
    mode.local()
    # ray_init("dell")
            ...
            * get_overlap_with_all_class
            ...
            with config:
                ...
                entry_points = get_entry_points(
                    ResNet18Cifar100.graph().load(), layer_num.value
                )
                ...
                path_template = (
                    "resnet_18_cifar100_real_metrics_per_layer_{0:.1f}_{1}_{2}.csv"
                )
                per_node = False
                resnet_18_overlap_ratio = resnet_18_cifar100_real_metrics_per_layer(
                    ...
                    class_trace_fn=lambda class_id: resnet_18_cifar100_class_trace_compact(
                        class_id,
                        threshold,
                        label=label,
                        variant=variant,
                    ...
                    path="metrics/"
                    + path_template.format(threshold, attack_name.value, label_name),
                    per_node=per_node,
