...
from nninst.backend.tensorflow.model import InceptionV4
...
def eval(batch_size: int = 64, label: str = None):
    ...
    if label is None:
        model_dir = abspath("tf/inception_v4/model/")
    else:
        model_dir = abspath(f"tf/inception_v4/model_{label}/")
    ...
    def eval_input_fn():
        return imagenet.test(
            ...
            preprocessing_fn=inception_preprocess_image,
        )
